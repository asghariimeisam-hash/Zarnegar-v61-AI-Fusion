...
function validation() {
  const sh=metrics('SHADOW'), lv=metrics('LIVE');
  const n=sh.n+lv.n, wins=(sh.wins||0)+(lv.wins||0);
  const wr=n?wins/n:null;
  const pf = (sh.pf && lv.pf) ? (sh.pf+lv.pf)/2 : (sh.pf||lv.pf);
  const maxDD=Math.max(sh.maxDD||0, lv.maxDD||0);
  const m={n,wins,losses:n-wins,wr,pf,maxDD};
...
function statBoard(){const sim=metrics('SIMULATION'),sh=metrics('SHADOW'),lv=metrics('LIVE');return `<section class="panel"><div class="title"><span class="kicker">REALITY CHECK</span><span>WR جداگانه</span></div><div class="metricBoard"><div><small>LIVE WR</small><b>${metricText(lv.wr,'pct')}</b><span>n=${lv.n}</span></div><div><small>SHADOW WR</small><b>${metricText(sh.wr,'pct')}</b><span>n=${sh.n}</span></div><div><small>SIM WR</small><b>${metricText(sim.wr,'pct')}</b><span>n=${sim.n}</span></div><div><small>SHADOW PF</small><b>${metricText(sh.pf)}</b><span>n=${sh.n}</span></div></div></section>`;}
